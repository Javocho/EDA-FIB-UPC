...
void readMatrix(Matrix& M, int n, int m, int& ncoins) {
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			cin >> M[i][j];
			if (M[i][j] > '0' and M[i][j] <= '9') ncoins++;
		}
	}
}
...
void bfsBishop(Matrix& M, MatrixBool& visited, int x, int y, int& coins, int& ncoins) {
	...
	while (not Q.empty()) {
		...
		for (auto p : dirBishop) {
			tmpi = i+p.first;
			tmpj = j+p.second;
			if (posOk(M, tmpi, tmpj) and !visited[tmpi][tmpj]) Q.push({tmpi, tmpj});
		}
	}
}

void bfsKnight(Matrix& M, MatrixBool& visited, int x, int y, int& coins, int& ncoins) {
	queue<pair<int,int>> Q;
	int i, j, tmpi, tmpj;
	Q.push({x,y});
	while (not Q.empty()) {
...
int searchCoins(Matrix& M, int n, int m, int &ncoins) {
	int coins = 0;
	MatrixBool visited(n, vector<bool>(m, false));
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (M[i][j] == 'B' or M[i][j] == 'K') {
				if (M[i][j] == 'B') bfsBishop(M, visited, i, j, coins, ncoins);
				else if (M[i][j] == 'K') bfsKnight(M, visited, i, j, coins, ncoins);
